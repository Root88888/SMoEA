...
Router 的工作：每筆進來的 query，在 150 個任務 adapter 中選出該由誰回答，
或判定「都不該接」而拒絕（拒絕後交給拒絕分支，見後文）。判定核心是
**共形 p 值**——query 與各任務的相似度領先差距（margin），放進離線建好的
校準分數庫中排名，換算成「這個領先程度在已知任務中有多罕見」的機率值，
再依 p 值分區決定去向：

| 判定順序 | 條件 | 去向 |
|---|---|---|
| 直判 | margin > 0.10（全域領先夠大） | 直接路由 |
| 紅區 | p < 0.02 | 拒絕 |
...
（Llama-3.1-8B）對前 3 名候選任務各答一題是非題「這筆 query 是不是
該任務的實例？」，最高信心 ≥ 0.5 就路由至該候選、三題都不像則拒絕，
