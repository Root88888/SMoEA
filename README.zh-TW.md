...
交付預設是 `data.routing_text=answer_free_full_prompt`：用移除答案後的完整請求建置
router，因為本專案 task0–14 的 `input` 欄位沒有完整的任務說明。訓練樣本的
`full_prompt` 尾端必須精確等於它的 `output`，建置時只移除這一段。測試資料與互動輸入
本來就不含答案，直接使用。

`router_assets_meta.json` 會記錄用的是哪一種 `routing_text`，避免兩套路由資產混淆。
要重現上游使用短 `input` 的 router，才明確加上 `--set data.routing_text=input`，並
建置到另一個 assets 目錄。
